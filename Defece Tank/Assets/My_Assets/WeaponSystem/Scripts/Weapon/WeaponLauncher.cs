...
public class WeaponLauncher : WeaponBase
{
    ...
    {
		None,Gun,MiniRocket,Rocket,Laser,MultiRocker,Napam
    }
	[SerializeField]WeaponType weaponType;
	private PoolManager_Defence poolManager;
	private void Start ()
	{
		if (!Owner) 
	...
	public void Shoot ()
	{
			...
			if (Time.time > nextFireTime + FireRate) {
				...
				for (int i = 0; i < NumBullet; i++) {
					...
					Vector3 direction = this.transform.forward;// + spread;
                        ...
                        {
							case WeaponType.Gun:
								bullet = poolManager.CurrentBullet();
								break;
							case WeaponType.Rocket:
								bullet = poolManager.CurrentRocket();
								break;
							case WeaponType.MiniRocket:
								bullet = poolManager.CurrentMiniRocket();
								break;
							case WeaponType.MultiRocker:

								break;
							case WeaponType.Napam:
		...
		if(weaponType==WeaponType.Rocket|| weaponType == WeaponType.MiniRocket)
        {
			BulletLoader.Instance.ShowLoader(FireRate);
		}
