...
Shader "Shader Sandwich/Enhanced Graphics/Mobile Skin" {//The Shaders Name
//The inputs shown in the material panel
Properties {
	_MainTex ("Texture", 2D) = "white" {}
	_Wrap_Amount ("Wrap Amount", Range(0.000000000,1.000000000)) = 0.758620700
	_Wrap_Color ("Wrap Color", Color) = (0.7490196,0.3215686,0.3215686,1)
	_Shininess ("Specular Hardness", Range(0.000100000,1.000000000)) = 0.400000000
}
...
half4 Diffuse ( VertexData vd, UnityGI gi, UnityGIInput giInput, inout half4 previousBaseColor){
	...
	float3 Lighting;
	half3 Surf1 =saturate(dot (vd.worldNormal, gi.light.dir));//Calculate lighting the standard way (See Diffuse lighting mode's comments).
	half3 Surf2 = saturate(dot (vd.worldNormal, gi.light.dir) + _Wrap_Amount);//Calculate diffuse lighting while taking the Wrap Amount into consideration.
	Unity_GlossyEnvironmentData g;
	g.roughness = _Wrap_Amount;
	g.reflUVW = vd.worldViewDir;
	gi = UnityGlobalIllumination(giInput, 1, vd.worldNormal,g);
	Lighting =  (Surf1+(Surf2*(_Wrap_Amount-abs(dot(vd.worldNormal, gi.light.dir)))*_Wrap_Amount * _Wrap_Color.rgb));//Combine the two lightings together, by adding the standard one with the wrapped one.
...
Shader Sandwich Shader
	...
	Begin Shader Base
		...
		Begin Shader Input
			Type(Text): "Range"
			VisName(Text): "Wrap Amount"
			Number(Float): 0.7586207
			Range0(Float): 0
			Range1(Float): 1
			CustomFallback(Text): "_Wrap_Amount"
		End Shader Input


		Begin Shader Input
			Type(Text): "Color"
			VisName(Text): "Wrap Color"
			Color(Vec): 0.7490196,0.3215686,0.3215686,1
			CustomFallback(Text): "_Wrap_Color"
		End Shader Input
		...
		ShaderName(Text): "Shader Sandwich/Enhanced Graphics/Mobile Skin"
		Tech Lod(Float): 200
		...
		Begin Shader Pass
			...
			Begin Shader Ingredient
				...
				Lighting Type(Type): Translucent - {TypeID = 4}
				Roughness Or Smoothness(Type): Smoothness - {TypeID = 0}
				Smoothness(Float): 0.4 - {Input = 3}
				Roughness(Float): 0.7586207 - {Input = 1}
				Light Size(Float): 0
				Wrap Amount(Float): 0.7586207 - {Input = 1}
				Wrap Color(Vec): 0.7490196,0.3215686,0.3215686,1 - {Input = 2}
				PBR Quality(Type): Low - {TypeID = 3}
			...
			Begin Shader Ingredient
				...
				Use Tangents(Toggle): False
				Use Ambient(Toggle): True
				Use Roughness Darkening(Toggle): False
			...
			Begin Shader Ingredient
				...
				Generate Shadow Caster Pass(Toggle): False
