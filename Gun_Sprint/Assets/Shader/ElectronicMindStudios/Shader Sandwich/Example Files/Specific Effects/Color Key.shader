...
Properties {
	...
	_Despill ("Despill", Range(0.000000000,1.000000000)) = 0.396761100
	_Bias ("Bias", Range(1.000000000,-1.000000000)) = 0.212500000
	_Hardness ("Hardness", Range(1.000000000,20.000000000)) = 13.468750000
	_Texture3_GLMain_Color ("Texture3 - Main Color", Color) = (0.2452422,0.6176471,0.34382,1)
}
...
half4 Unlit ( VertexData vd){
	...
	Surface = half4(TextureSurface_Sample1.rgb,1) ;//0
	
	
			//Generate Layer: Texture3
				//Sample parts of the layer:
					half4 Texture3Surface_Sample1 = _Texture3_GLMain_Color;
	
	Surface = lerp(Surface,half4((Surface.rgb - Texture3Surface_Sample1.rgb), 1),_Despill * vd.Mask0);//2
	...
	float Surface = 0;
		//Generate layers for the Transparency channel.
			//Generate Layer: Alpha2
				//Sample parts of the layer:
					half4 Alpha2Transparency_Sample1 = GammaToLinear(float4(1, 1, 1, 1));
	
	Surface = Alpha2Transparency_Sample1.r;//2
	
	
			//Generate Layer: Alpha3
				//Sample parts of the layer:
					half4 Alpha3Transparency_Sample1 = GammaToLinear(float4(0, 0, 0, 1));
	
	Surface = lerp(Surface,Alpha3Transparency_Sample1.r,vd.Mask0);//1
	
	
			//Generate Layer: Mask0 Copy
				//Sample parts of the layer:
					half4 Mask0_CopyTransparency_Sample1 = float4(Surface.rrrr);
...
float Mask_Mask0 ( VertexData vd){
		...
		//Generate layers for the Mask0 channel.
			//Generate Layer: Mask0 Copy Copy
				...
				//Apply Effects:
					Mask0_Copy_CopyMask0_Sample1.rgb = ((distance(Mask0_Copy_CopyMask0_Sample1.xyz,float3(_Texture3_GLMain_Color.r,_Texture3_GLMain_Color.g,_Texture3_GLMain_Color.b))).rrrr);
...
Shader Sandwich Shader
	...
	Begin Shader Base
		...
		Begin Shader Input
			Type(Text): "Range"
			VisName(Text): "Despill"
			Number(Float): 0.3967611
			Range0(Float): 0
			Range1(Float): 1
			CustomFallback(Text): "_Despill"
		End Shader Input
		...
		Begin Shader Input
			Type(Text): "Color"
			VisName(Text): "Texture3 - Main Color"
			Color(Vec): 0.2452422,0.6176471,0.34382,1
			CustomFallback(Text): "_Texture3_GLMain_Color"
		End Shader Input

		ShaderName(Text): "Shader Sandwich/Specific/Color Key"
		Tech Lod(Float): 200
			...
			Begin Shader Layer List
				...
				Begin Shader Layer
					...
					Begin Shader Effect
						...
						X(Float): 0.2452422 - {ColorComponent = 0, Input = 4}
						Y(Float): 0.6176471 - {ColorComponent = 1, Input = 4}
						Z(Float): 0.34382 - {ColorComponent = 2, Input = 4}
						W(Float): 0
					End Shader Effect
			...
			Begin Shader Ingredient
				...
				Begin Shader Layer List
					...
					Begin Shader Layer
						...
						Use Alpha(Toggle): False
						Alpha Blend Mode(Type): Blend - {TypeID = 0}
						Mix Amount(Float): 0.3967611 - {Input = 1}
						Mix Type(Type): Subtract - {TypeID = 2}
						Stencil(ObjectArray): Mask0 - {ObjectID = 0}
						Color(Vec): 0.2452422,0.6176471,0.34382,1 - {Input = 4}
						Cubemap(Cubemap): 
						Noise Dimensions(Type): SSN/A - {TypeID = 0}
						Color 2(Vec): 0,0,0,1
						Texture(Texture): 
				...
				Begin Shader Layer List
					...
					Begin Shader Layer
						...
						Begin Shader Effect
							...
							IsVisible(Toggle): True
							UseAlpha(Float): 0
							Contrast(Float): 13.46875 - {Input = 3}
						End Shader Effect
