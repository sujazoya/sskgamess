...
Shader "Shader Sandwich/Specific/Edge Faded Water Legacy" {//The Shaders Name
		...
		//Generate layers for the Albedo channel.
			//Generate Layer: Texture2
				//Sample parts of the layer:
					half4 Texture2Albedo_Sample1 = _Underwater_Color;
...
half4 Set_Alpha_Channel ( VertexData vd, half4 outputColor){
	...
	Surface = Legacy_Transparency_BaseTransparency_Sample1.r;//2
			...
			//Generate Layer: Alpha
				//Sample parts of the layer:
					half4 AlphaTransparency_Sample1 = (LinearEyeDepth(tex2D(_CameraDepthTexture, SillyFlip(vd.screenPos.xyz.xy)).r).rrrr);
	
	Surface = AlphaTransparency_Sample1.r;//0
	
	
			//Generate Layer: Alpha2
				//Sample parts of the layer:
					half4 Alpha2Transparency_Sample1 = float4(vd.screenPos.xyz,1);
	
				//Apply Effects:
					Alpha2Transparency_Sample1.rgb = Alpha2Transparency_Sample1.bbb;
	
	Surface = (Surface - Alpha2Transparency_Sample1.r);//0
	
	
			//Generate Layer: Alpha3
				//Sample parts of the layer:
					half4 Alpha3Transparency_Sample1 = float4(Surface.rrrr);
	
				//Apply Effects:
					Alpha3Transparency_Sample1.rgb = (Alpha3Transparency_Sample1.rgb*_Water_Density);
					Alpha3Transparency_Sample1.rgb = clamp(Alpha3Transparency_Sample1.rgb,0,1);
					Alpha3Transparency_Sample1.rgb = (Alpha3Transparency_Sample1.rgb*1);
	
	Surface = Alpha3Transparency_Sample1.r;//0
	...
	half3 Surface = half4(0,0,0,1);//Specular Mode
		//Generate layers for the Specular Color channel.
			//Generate Layer: Specular Color
				//Sample parts of the layer:
					half4 Specular_ColorSpecular_Sample1 = GammaToLinear(float4(0.3676471, 0.3676471, 0.3676471, 1));
...
Shader Sandwich Shader
	...
	Begin Shader Base
		...
		ShaderName(Text): "Shader Sandwich/Specific/Edge Faded Water Legacy"
		Tech Lod(Float): 200
			...
			Begin Shader Ingredient
				...
				Begin Shader Layer List
					...
					Begin Shader Layer
						...
						Color(Vec): 0.666144,0.7677045,0.8161765,1 - {Input = 0}
						Cubemap(Cubemap): 
				...
				Begin Shader Layer List
					...
					Begin Shader Layer
						...
						Begin Shader Effect
							...
							IsVisible(Toggle): True
							UseAlpha(Float): 0
							X Offset(Float): 108.0457 - {Input = 5}
							Y Offset(Float): 0
					...
					Begin Shader Layer
						...
						Begin Shader Effect
							...
							UseAlpha(Float): 0
							X Offset(Float): 0
							Y Offset(Float): 108.0457 - {Input = 5}
							Z Offset(Float): 0
						End Shader Effect
			...
			Begin Shader Ingredient
				...
				Begin Shader Layer List
					...
					Begin Shader Layer
						...
						Mix Type(Type): Mix - {TypeID = 0}
						Stencil(ObjectArray): SSNone - {ObjectID = -2}
						Color(Vec): 0.3676471,0.3676471,0.3676471,1
					End Shader Layer
