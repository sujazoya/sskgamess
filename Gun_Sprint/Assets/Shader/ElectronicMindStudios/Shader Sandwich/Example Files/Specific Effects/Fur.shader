...
Properties {
	...
	_Depth_Darkening_AO ("Depth Darkening (AO)", Color) = (0,0,0,0.697)
}
...
half4 Diffuse ( VertexData vd, UnityGI gi, UnityGIInput giInput){
	...
	Surface = half4(Texture_2Albedo_Sample1.rgb,1);//7
			...
			//Generate Layer: Texture Copy 2 2
				//Sample parts of the layer:
					half4 Texture_Copy_2_2Albedo_Sample1 = _Depth_Darkening_AO;
	
	Surface = lerp(Surface,half4(Texture_Copy_2_2Albedo_Sample1.rgb, 1),Texture_Copy_2_2Albedo_Sample1.a);//2
...
half4 Set_Alpha_Channel ( half4 outputColor){
	float Surface = 0;
		//Generate layers for the Transparency channel.
			//Generate Layer: Legacy Transparency Base 2
				//Sample parts of the layer:
					half4 Legacy_Transparency_Base_2Transparency_Sample1 = 1;
	
	Surface = Legacy_Transparency_Base_2Transparency_Sample1.a;//2
	...
	half4 Surface = half4(0,0,0,0);
		//Generate layers for the Albedo channel.
			//Generate Layer: Texture Copy 2
				//Sample parts of the layer:
					half4 Texture_Copy_2Albedo_Sample1 = tex2D(_MainTex,vd.uv_MainTex);
	
	Surface = half4(Texture_Copy_2Albedo_Sample1.rgb,1);//7
	
	
			//Generate Layer: Texture
				//Sample parts of the layer:
					half4 TextureAlbedo_Sample1 = _Depth_Darkening_AO;
	
	Surface = lerp(Surface,half4(TextureAlbedo_Sample1.rgb, 1),TextureAlbedo_Sample1.a * 0.8888889);//2
...
void Shells ( inout VertexShaderInput v){
	...
	v.vertex.xyz = float3(v.vertex.xyz + v.normal * 0.1111111 * _ShellDistance);
...
half4 Diffuse ( VertexData vd, UnityGI gi, UnityGIInput giInput){
	...
	Surface = lerp(Surface,half4(TextureAlbedo_Sample1.rgb, 1),TextureAlbedo_Sample1.a * 0.7777778);//2
...
void Shells ( inout VertexShaderInput v){
	...
	v.vertex.xyz = float3(v.vertex.xyz + v.normal * 0.2222222 * _ShellDistance);
...
half4 Diffuse ( VertexData vd, UnityGI gi, UnityGIInput giInput){
	...
	Surface = lerp(Surface,half4(TextureAlbedo_Sample1.rgb, 1),TextureAlbedo_Sample1.a * 0.6666667);//2
...
void Shells ( inout VertexShaderInput v){
	...
	v.vertex.xyz = float3(v.vertex.xyz + v.normal * 0.3333333 * _ShellDistance);
...
half4 Diffuse ( VertexData vd, UnityGI gi, UnityGIInput giInput){
	...
	Surface = lerp(Surface,half4(TextureAlbedo_Sample1.rgb, 1),TextureAlbedo_Sample1.a * 0.5555556);//2
...
void Shells ( inout VertexShaderInput v){
	...
	v.vertex.xyz = float3(v.vertex.xyz + v.normal * 0.4444444 * _ShellDistance);
...
half4 Diffuse ( VertexData vd, UnityGI gi, UnityGIInput giInput){
	...
	Surface = lerp(Surface,half4(TextureAlbedo_Sample1.rgb, 1),TextureAlbedo_Sample1.a * 0.4444444);//2
...
void Shells ( inout VertexShaderInput v){
	...
	v.vertex.xyz = float3(v.vertex.xyz + v.normal * 0.5555556 * _ShellDistance);
...
half4 Diffuse ( VertexData vd, UnityGI gi, UnityGIInput giInput){
	...
	Surface = lerp(Surface,half4(TextureAlbedo_Sample1.rgb, 1),TextureAlbedo_Sample1.a * 0.3333333);//2
...
void Shells ( inout VertexShaderInput v){
	...
	v.vertex.xyz = float3(v.vertex.xyz + v.normal * 0.6666666 * _ShellDistance);
...
half4 Diffuse ( VertexData vd, UnityGI gi, UnityGIInput giInput){
	...
	Surface = lerp(Surface,half4(TextureAlbedo_Sample1.rgb, 1),TextureAlbedo_Sample1.a * 0.2222222);//2
...
void Shells ( inout VertexShaderInput v){
	...
	v.vertex.xyz = float3(v.vertex.xyz + v.normal * 0.7777778 * _ShellDistance);
...
half4 Diffuse ( VertexData vd, UnityGI gi, UnityGIInput giInput){
	...
	Surface = lerp(Surface,half4(TextureAlbedo_Sample1.rgb, 1),TextureAlbedo_Sample1.a * 0.1111111);//2
...
void Shells ( inout VertexShaderInput v){
	...
	v.vertex.xyz = float3(v.vertex.xyz + v.normal * 0.8888889 * _ShellDistance);
...
half4 Diffuse ( VertexData vd){
	...
	Surface = lerp(Surface,half4(TextureAlbedo_Sample1.rgb, 1),TextureAlbedo_Sample1.a);//2
...
Shader Sandwich Shader
	...
	Begin Shader Base
		...
		Begin Shader Input
			Type(Text): "Image"
			VisName(Text): "Texture - Main Texture"
			ImageDefault(Float): 0
			Image(Text): "824bbecff5e03c7449eea0b94db3c76b"
			NormalMap(Float): 0
			DefaultTexture(Text): "White"
			SeeTilingOffset(Toggle): True
			TilingOffset(Vec): 4,4,0,0
		...
		Begin Shader Input
			Type(Text): "Color"
			VisName(Text): "Depth Darkening (AO)"
			Color(Vec): 0,0,0,0.697
			CustomFallback(Text): "_Depth_Darkening_AO"
		End Shader Input


		Begin Shader Input
			Type(Text): "Range"
			VisName(Text): "Shell Dist"
			Number(Float): 1
			Range0(Float): 0
			Range1(Float): 1
		...
		ShaderName(Text): "Shader Sandwich/Specific/Fur"
		Tech Lod(Float): 200
		...
		Begin Shader Pass
			...
			Begin Shader Ingredient
				...
				Begin Shader Layer List
					...
					Begin Shader Layer
						...
						Texture(Texture): 824bbecff5e03c7449eea0b94db3c76b - {Input = 0}
						Cubemap(Cubemap): 
					...
					Begin Shader Layer
						Layer Name(Text): "Texture Copy 2 2"
						...
						Use Alpha(Toggle): True
						Alpha Blend Mode(Type): Blend - {TypeID = 0}
						Mix Amount(Float): 1 - {Input = 3}
						Mix Type(Type): Mix - {TypeID = 0}
						Stencil(ObjectArray): SSNone - {ObjectID = -1}
						Color(Vec): 0,0,0,0.697 - {Input = 2}
						Cubemap(Cubemap): 
						Noise Dimensions(Type): SSN/A - {TypeID = 0}
						Color 2(Vec): 0,0,0,1
						Texture(Texture): 
			...
			Begin Shader Ingredient
				...
				Begin Shader Layer List
					...
					Begin Shader Layer
						Layer Name(Text): "Legacy Transparency Base 2"
		...
		Begin Shader Pass
			...
			Begin Shader Ingredient
				...
				Begin Shader Layer List
					...
					Begin Shader Layer
						Layer Name(Text): "Texture Copy 2"
						Layer Type(ObjectArray): SLTTexture - {ObjectID = 1}
						UV Map(Type): UV Map - {TypeID = 0}
						Map Local(Toggle): False
						Map Space(Type): World - {TypeID = 0}
