...
Shader "Shader Sandwich/Specific/Simple Wax" {//The Shaders Name
//The inputs shown in the material panel
Properties {
	_Color ("Color", Color) = (0.5588235,0.4457553,0.3246107,1)
	_MainTex ("Texture", 2D) = "white" {}
	_Distance ("Distance", Float) = 2.540000000
	_Falloff ("Falloff", Float) = 4.060000000
	_Wax_Level ("Wax Level", Range(0.000000000,1.000000000)) = 0.391752600
}
...
half4 Emission ( VertexData vd, half4 outputColor){
	half4 Surface = half4(0,0,0,0);
		//Generate layers for the Emission channel.
			//Generate Layer: Scattering
				//Sample parts of the layer:
					half4 ScatteringEmission_Sample1 = float4(mul(unity_WorldToObject, float4(vd.worldPos,1)).xyz,1);
	
				//Apply Effects:
					ScatteringEmission_Sample1.rgb = ((distance(ScatteringEmission_Sample1.xyz,float3(0,0,0))).rrrr);
					ScatteringEmission_Sample1.rgb = pow(ScatteringEmission_Sample1.rgb,_Falloff);
					ScatteringEmission_Sample1.rgb = (ScatteringEmission_Sample1.rgb*_Distance);
					ScatteringEmission_Sample1.rgb = clamp(ScatteringEmission_Sample1.rgb,0,2);
	
	Surface = lerp(Surface,half4((Surface.rgb + ScatteringEmission_Sample1.rgb), 1),_Wax_Level);//2
	
	
			//Generate Layer: Emission Multiply
				//Sample parts of the layer:
					half4 Emission_MultiplyEmission_Sample1 = outputColor;
	
	Surface = half4((Surface.rgb * Emission_MultiplyEmission_Sample1.rgb),1) ;//0
...
// Setup lighting environment
				...
				outputColor = half4(outputDiffuse.rgb,1);//7
								half4 outputEmission = Emission ( vd, outputColor);
...
Shader Sandwich Shader
	...
	Begin Shader Base
		...
		Begin Shader Input
			Type(Text): "Color"
			VisName(Text): "Color"
			Color(Vec): 0.5588235,0.4457553,0.3246107,1
		...
		Begin Shader Input
			Type(Text): "Float"
			VisName(Text): "Distance"
			Number(Float): 2.54
			Range0(Float): 0
			Range1(Float): 1
			CustomFallback(Text): "_Distance"
		End Shader Input


		Begin Shader Input
			Type(Text): "Float"
			VisName(Text): "Falloff"
			Number(Float): 4.06
			Range0(Float): 0
			Range1(Float): 1
			CustomFallback(Text): "_Falloff"
		End Shader Input


		Begin Shader Input
			Type(Text): "Range"
			VisName(Text): "Wax Level"
			Number(Float): 0.3917526
			Range0(Float): 0
			Range1(Float): 1
			CustomFallback(Text): "_Wax_Level"
		End Shader Input

		ShaderName(Text): "Shader Sandwich/Specific/Simple Wax"
		Tech Lod(Float): 200
			...
			Begin Shader Ingredient
				...
				Begin Shader Layer List
					...
					Begin Shader Layer
						...
						Stencil(ObjectArray): SSNone - {ObjectID = -1}
						Color(Vec): 0.5588235,0.4457553,0.3246107,1 - {Input = 0}
						Cubemap(Cubemap): 
						Noise Dimensions(Type): SSN/A - {TypeID = 0}
						Color 2(Vec): 0,0,0,1
						Texture(Texture): 
			...
			Begin Shader Ingredient
				...
				Begin Shader Layer List
					...
					Begin Shader Layer
						...
						Use Alpha(Toggle): False
						Alpha Blend Mode(Type): Blend - {TypeID = 0}
						Mix Amount(Float): 0.3917526 - {Input = 4}
						Mix Type(Type): Add - {TypeID = 1}
						Stencil(ObjectArray): SSNone - {ObjectID = -1}
						Cubemap(Cubemap): 
						Noise Dimensions(Type): SSN/A - {TypeID = 0}
						...
						Square(Toggle): False
						Begin Shader Effect
							TypeS(Text): "SSEMathDistance"
							IsVisible(Toggle): True
							UseAlpha(Float): 0
							Dimensions(Type): 3D - {TypeID = 2}
							X(Float): 0
							Y(Float): 0
							Z(Float): 0
							W(Float): 0
					...
					Begin Shader Layer
						Layer Name(Text): "Emission Multiply"
						Layer Type(ObjectArray): SLTCurrentOutputColor - {ObjectID = 18}
						UV Map(Type): UV Map - {TypeID = 0}
						Map Local(Toggle): False
						Map Space(Type): World - {TypeID = 0}
						Map Generate Space(Type): Object - {TypeID = 1}
