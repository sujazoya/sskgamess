			...
			PoissonBlurRotationAnimated = float4(1,0,0,1);
				...
				if (Quality>=50){
					...
					for(float ii = 0; ii < 50; ii += 1){
						float2 asd = Poisson50Points[ii];
						if (Weighting_Inverse==0) asd = normalize(Poisson50Points[ii]) * (1.0-pow(1.0-length(Poisson50Points[ii]),Factor));
						else asd = normalize(Poisson50Points[ii]) * (pow(length(Poisson50Points[ii]),Factor));
						newColor+=tex2Dlod(_Previous,float4(i.uv+float2(dot(asd,PoissonBlurRotationAnimated.xz),dot(asd,PoissonBlurRotationAnimated.yw))*Blur,0,0))*(1.0/50.0);
					}
				}else if (Quality>=40){
					[unroll]
					for(float ii = 0; ii < 40; ii += 1){
						float2 asd = Poisson40Points[ii];
						if (Weighting_Inverse==0) asd = normalize(Poisson40Points[ii]) * (1.0-pow(1.0-length(Poisson40Points[ii]),Factor));
						else asd = normalize(Poisson40Points[ii]) * (pow(length(Poisson40Points[ii]),Factor));
						newColor+=tex2Dlod(_Previous,float4(i.uv+float2(dot(asd,PoissonBlurRotationAnimated.xz),dot(asd,PoissonBlurRotationAnimated.yw))*Blur,0,0))*(1.0/40.0);
					}
				}else if (Quality>=30){
					[unroll]
					for(float ii = 0; ii < 30; ii += 1){
						float2 asd = Poisson30Points[ii];
						if (Weighting_Inverse==0) asd = normalize(Poisson30Points[ii]) * (1.0-pow(1.0-length(Poisson30Points[ii]),Factor));
						else asd = normalize(Poisson30Points[ii]) * (pow(length(Poisson30Points[ii]),Factor));
						newColor+=tex2Dlod(_Previous,float4(i.uv+float2(dot(asd,PoissonBlurRotationAnimated.xz),dot(asd,PoissonBlurRotationAnimated.yw))*Blur,0,0))*(1.0/30.0);
					}
				}else if (Quality>=25){
					[unroll]
					for(float ii = 0; ii < 25; ii += 1){
						float2 asd = Poisson25Points[ii];
						if (Weighting_Inverse==0) asd = normalize(Poisson25Points[ii]) * (1.0-pow(1.0-length(Poisson25Points[ii]),Factor));
						else asd = normalize(Poisson25Points[ii]) * (pow(length(Poisson25Points[ii]),Factor));
						newColor+=tex2Dlod(_Previous,float4(i.uv+float2(dot(asd,PoissonBlurRotationAnimated.xz),dot(asd,PoissonBlurRotationAnimated.yw))*Blur,0,0))*(1.0/25.0);
					}
				}else if (Quality>=20){
					[unroll]
					for(float ii = 0; ii < 20; ii += 1){
						float2 asd = Poisson20Points[ii];
						if (Weighting_Inverse==0) asd = normalize(Poisson20Points[ii]) * (1.0-pow(1.0-length(Poisson20Points[ii]),Factor));
						else asd = normalize(Poisson20Points[ii]) * (pow(length(Poisson20Points[ii]),Factor));
						newColor+=tex2Dlod(_Previous,float4(i.uv+float2(dot(asd,PoissonBlurRotationAnimated.xz),dot(asd,PoissonBlurRotationAnimated.yw))*Blur,0,0))*(1.0/20.0);
					}
				}else if (Quality>=15){
					[unroll]
					for(float ii = 0; ii < 15; ii += 1){
						float2 asd = Poisson15Points[ii];
						if (Weighting_Inverse==0) asd = normalize(Poisson15Points[ii]) * (1.0-pow(1.0-length(Poisson15Points[ii]),Factor));
						else asd = normalize(Poisson15Points[ii]) * (pow(length(Poisson15Points[ii]),Factor));
						newColor+=tex2Dlod(_Previous,float4(i.uv+float2(dot(asd,PoissonBlurRotationAnimated.xz),dot(asd,PoissonBlurRotationAnimated.yw))*Blur,0,0))*(1.0/15.0);
					}
				}else if (Quality>=10){
					[unroll]
					for(float ii = 0; ii < 10; ii += 1){
						float2 asd = Poisson10Points[ii];
						if (Weighting_Inverse==0) asd = normalize(Poisson10Points[ii]) * (1.0-pow(1.0-length(Poisson10Points[ii]),Factor));
						else asd = normalize(Poisson10Points[ii]) * (pow(length(Poisson10Points[ii]),Factor));
						newColor+=tex2Dlod(_Previous,float4(i.uv+float2(dot(asd,PoissonBlurRotationAnimated.xz),dot(asd,PoissonBlurRotationAnimated.yw))*Blur,0,0))*(1.0/10.0);
					}
				}else if (Quality>=5){
					[unroll]
					for(float ii = 0; ii < 5; ii += 1){
						float2 asd = Poisson5Points[ii];
						if (Weighting_Inverse==0) asd = normalize(Poisson5Points[ii]) * (1.0-pow(1.0-length(Poisson5Points[ii]),Factor));
						else asd = normalize(Poisson5Points[ii]) * (pow(length(Poisson5Points[ii]),Factor));
						newColor+=tex2Dlod(_Previous,float4(i.uv+float2(dot(asd,PoissonBlurRotationAnimated.xz),dot(asd,PoissonBlurRotationAnimated.yw))*Blur,0,0))*(1.0/5.0);
					}
				}else if (Quality>=4){
					[unroll]
					for(float ii = 0; ii < 4; ii += 1){
						float2 asd = Poisson4Points[ii];
						if (Weighting_Inverse==0) asd = normalize(Poisson4Points[ii]) * (1.0-pow(1.0-length(Poisson4Points[ii]),Factor));
						else asd = normalize(Poisson4Points[ii]) * (pow(length(Poisson4Points[ii]),Factor));
						newColor+=tex2Dlod(_Previous,float4(i.uv+float2(dot(asd,PoissonBlurRotationAnimated.xz),dot(asd,PoissonBlurRotationAnimated.yw))*Blur,0,0))*(1.0/4.0);
					}
				}else if (Quality>=3){
					[unroll]
					for(float ii = 0; ii < 3; ii += 1){
						float2 asd = Poisson3Points[ii];
						if (Weighting_Inverse==0) asd = normalize(Poisson3Points[ii]) * (1.0-pow(1.0-length(Poisson20Points[ii]),Factor));
						else asd = normalize(Poisson3Points[ii]) * (pow(length(Poisson3Points[ii]),Factor));
						newColor+=tex2D(_Previous,i.uv+float2(dot(asd,PoissonBlurRotationAnimated.xz),dot(asd,PoissonBlurRotationAnimated.yw))*Blur)*(1.0/3.0);
					}
				}
