...
public class GamePlayManager : MonoBehaviour {
    ...
    {
		StartCoroutine(TryAddEvent());
    }
	IEnumerator TryAddEvent()
    {
		yield return new WaitUntil(() => AdmobAdmanager.readyToShoAd);
		Timer.Schedule(this, 2f, AddEvents);
	}

    private void AddEvents()
    {
#if UNITY_ANDROID || UNITY_IOS
        if (AdmobAdmanager.readyToShoAd==true)
        {
			RewardedAdManager.CurrentRewardedAd().OnUserEarnedReward += HandleRewardBasedVideoRewarded;
			RewardedAdManager.CurrentRewardedAd().OnAdClosed += HandleRewardBasedVideoClosed;
        }
#endif
    }
    ...
    {
        if (AdmobAdmanager.readyToShoAd == false) return false;
...
//#endif
	...
	public void BackToHome()
	{
		//CUtils.ShowInterstitialAd();
		SoundManager.instance.PlaybtnSfx ();
		Level_Loader.instance.LoadLevel("HomeScene");
