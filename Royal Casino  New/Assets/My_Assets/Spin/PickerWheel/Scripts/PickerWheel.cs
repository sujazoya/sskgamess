...
namespace EasyUI.PickerWheelUI {
   ...
   public class PickerWheel : MonoBehaviour {
      ...
      private void DrawPiece (int index) {
         ...
         pieceTrns.GetChild (2).GetComponent <Image> ().sprite = piece.Icon ;
         pieceTrns.GetChild (1).GetComponent <Text> ().text = piece.Label ;
         pieceTrns.GetChild (0).GetComponent <Text> ().text = piece.Amount.ToString () ;
      ...
      public void Spin () {
            if (!_isSpinning)
            {
                ...
                if (spinButtonAnimator) { spinButtonAnimator.enabled = false; }
            }
            else
            {
                MessegeManager.Instance.ShowMassege("Sorry!", "You Have 0 Spin Left", false);
            }           
